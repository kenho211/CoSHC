...
class BiEncoderModel(RobertaPreTrainedModel):
    ...
    def forward(self, code_inputs, query_inputs, labels=None):
        ...
        code_rep = code_outputs.last_hidden_state[:, 0, :]
        query_rep = query_outputs.last_hidden_state[:, 0, :]
        
        # Cosine similarity calculation
        cos_sim = torch.nn.functional.cosine_similarity(code_rep, query_rep, dim=-1)
        
        loss = None
        if labels is not None:
            loss_fct = torch.nn.CosineEmbeddingLoss()
            loss = loss_fct(code_rep, query_rep, labels*2-1)  # Convert 0/1 labels to -1/1
        
        # Return cos_sim as primary output
        return (loss, cos_sim) if loss is not None else (cos_sim,)


MODEL_CLASSES = {'roberta': (RobertaConfig, BiEncoderModel, RobertaTokenizer)}
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                preds = np.append(preds, cos_sim, axis=0)
...
def load_and_cache_examples(args, task, tokenizer, ttype='train'):
    ...
    all_code_input_ids = torch.tensor([f.code_input_ids for f in features], dtype=torch.long)
    all_code_input_mask = torch.tensor([f.code_input_mask for f in features], dtype=torch.long)
    all_code_segment_ids = torch.tensor([f.code_segment_ids for f in features], dtype=torch.long)
    all_query_input_ids = torch.tensor([f.query_input_ids for f in features], dtype=torch.long)
    all_query_input_mask = torch.tensor([f.query_input_mask for f in features], dtype=torch.long)
    all_query_segment_ids = torch.tensor([f.query_segment_ids for f in features], dtype=torch.long)
    if output_mode == "classification":
        all_label_ids = torch.tensor([f.label_id for f in features], dtype=torch.long)

    dataset = TensorDataset(all_code_input_ids, all_code_input_mask, all_code_segment_ids, all_query_input_ids, all_query_input_mask, all_query_segment_ids, all_label_ids)
    if (ttype == 'test'):
